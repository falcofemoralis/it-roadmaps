...
export default class RoadmapController {
    ...
    public saveRoadmapData = async (req: any, res: any) => {
        try {
            await this.roadmapModel.addNodes(req.body)
    ...
    public updateRoadmapData = async (req: any, res: any) => {
        try {
            await this.roadmapModel.updateNodes(req.body);
            res.status(HttpCodes.OK).send();
        } catch (err) {
            res.status(HttpCodes.InternalServerError).send(err);
        }
